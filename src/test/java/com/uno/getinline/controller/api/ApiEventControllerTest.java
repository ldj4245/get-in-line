...
class ApiEventControllerTest {
    ...
    void givenEventId_whenRequestingExistentEvent_thenReturnsEventInStandardResponse()throws  Exception{
        ...
        mvc.perform(get("/api/events/" + eventId))
                ...
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.errorCode").value(ErrorCode.OK.getCode()))
                .andExpect(jsonPath("$.massage").value(ErrorCode.OK.getMessage()));
   ...
   @Test
   void givenWrongEventID_whenRequestingNonexistentEvent_thenReturnsFailedStandardRespons() throws Exception{
        //Given
       long eventId = 0L;

       //When & Then
       mvc.perform(get("/api/evetns"+eventId))
               .andExpect(status().isBadRequest())
               .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
               .andExpect(jsonPath("$.success").value(false))
               .andExpect(jsonPath("$.errorCode").value(ErrorCode.VALIDATION_ERROR.getCode()))
   ...
   @Test
   void givenEventId_whenDeletingAnEvent_thenReturnsSuccessfulStandResponse() throws Exception{
        //Given
       long eventId =1L;
